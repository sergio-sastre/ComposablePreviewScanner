...
1. All Packages: `scanAllPackages()`. This might require a huge amount of memory since it would scan not only in a set of packages, but in all packages used in your app/module (i.e. also in its dependencies). This is in 99% of the cases unnecessary, and scanning the main package trees of your module should be sufficient. 
2. From a file for faster times: 
   2.1. `scanFile(jsonFile: File)`. The file had been previously be generated by using ScanResultDumper().dumpScanResultToFile(jsonFile)
...
class PreviewParameterizedTests(
    ...
    companion object {
        ...
        fun values(): List<ComposePreview<AndroidPreviewInfo>> =
            AndroidComposablePreviewsScanner()
                .scanPackageTrees("your.package", "your.package2")
...
Let's say we want to enable some custom Dropshots Config for some Previews, for instance a maxPercentDifferent value.
   ...
   class PreviewParameterizedTests(
      ...
      companion object {
         ...
         fun values(): List<AndroidComposePreview<PreviewInfo>> =
               AndroidComposablePreviewScanner()
